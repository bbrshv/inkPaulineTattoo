...
export async function GET(): Promise<NextResponse<HeroResponse>> {
  ...
  // Заглушка для dev без Cloudinary
  if (!cloudName) {
    const fallback =
  ...
  const videoUrl = `https://res.cloudinary.com/${cloudName}/video/upload/q_auto/f_auto/v1777910645/mainBackground.mov`;
  const imageUrl = `https://res.cloudinary.com/${cloudName}/image/upload/q_auto/f_auto/v1777902076/mainBackground`;
  // Первый кадр видео: Cloudinary отдаёт JPG из видео через so_0 (start offset 0)
  const posterUrl = `https://res.cloudinary.com/${cloudName}/video/upload/so_0,q_auto,f_auto/v1777910645/mainBackground.jpg`;

  // Проверяем, что видео вообще доступно
  let videoAvailable = videoUrl;
  try {
    const response = await fetch(videoUrl, { method: "HEAD" });
    if (!response.ok) videoAvailable = "";
  } catch {
    videoAvailable = "";
  }
